...
export class Airboat extends THREE.Object3D {
  ...
  constructor(world: PLANCK.World) {
    ...
    const engineGeometry = new THREE.CylinderGeometry(0.01, 0.01, 0.03).rotateZ(
      Math.PI / 2,
    );
    const rudderGeometry = new THREE.BoxGeometry(0.04, 0.14, 0.002).translate(
      -0.02,
      0,
      0,
    );
    const wakeGeometry = new THREE.PlaneGeometry(
      this.settings.wakeLength,
      0.12,
    ).rotateX(-Math.PI / 2);
    ...
    this.engine = new THREE.Mesh(
      engineGeometry,
      new THREE.MeshLambertMaterial({ color: this.settings.lineColor }),
    );
    this.wake = new THREE.Mesh(wakeGeometry, this.wakeMaterial);
    this.foam = new THREE.Points(
      this.generateFoamGeometry(),
      this.foamMaterial,
    );
    ...
    this.rudder.add(
      new THREE.LineSegments(
        new THREE.EdgesGeometry(rudderGeometry),
        this.lineMaterial,
      ),
    );
    this.propMount.add(
      new THREE.LineSegments(
        new THREE.EdgesGeometry(propMountGeometry),
        this.lineMaterial,
      ),
    );
  ...
  public updateCamera(camera: THREE.PerspectiveCamera, angle: number) {
    const cameraDistance = Math.max(
      ...
      this.settings.baseCameraDistance *
        ...
        this.settings.cameraDistanceVelocityScale,
    );
    ...
    camera.position.copy(
      cameraOffset.applyQuaternion(this.quaternion).add(this.position),
    );
  ...
  public calculateForces(axisValues: axisValues) {
    ...
    // Steering
    const turningTorque =
      Math.sin((-axisValues.yaw * Math.PI) / 2) *
      (axisValues.throttle * this.settings.thrustTurningTorque +
        this.body.getLinearVelocity().length() *
          this.settings.velocityTurningTorque);
    const turningFriction =
      this.settings.turningFriction * this.body.getAngularVelocity();
    this.body.applyTorque(turningTorque - turningFriction);

    // Drag
    const localDrag = this.body.getLocalVector(
      this.body.getLinearVelocity().clone().neg(),
    );
  ...
  public syncBodyAndMesh() {
    ...
    if (this.debugArrow) {
      const vel = this.body.getLocalVector(
        this.body.getLinearVelocity().clone(),
      );
      vel.normalize();
  ...
  private applySway(force: PLANCK.Vec2) {
    this.hull.rotation.x = -THREE.MathUtils.clamp(
      force.y * this.settings.swayMultiplierX,
      -this.settings.swayMaxX,
      this.settings.swayMaxX,
    );
    this.hull.rotation.z = THREE.MathUtils.clamp(
      force.x * this.settings.swayMultiplierZ,
      -this.settings.swayMaxZ,
      this.settings.swayMaxZ,
    );
  }
  ...
  private generateFoamGeometry() {
    ...
    geometry.setAttribute(
      'position',
      new THREE.Float32BufferAttribute(foamPosition, 3),
    );
    geometry.setAttribute(
      'jumpHeight',
      new THREE.Float32BufferAttribute(foamJumpHeight, 1),
    );
    geometry.setAttribute(
      'jumpLength',
      new THREE.Float32BufferAttribute(foamJumpLength, 1),
    );
    geometry.setAttribute(
      'jumpSpeed',
      new THREE.Float32BufferAttribute(foamJumpSpeed, 1),
    );
    geometry.setAttribute(
      'jumpStop',
      new THREE.Float32BufferAttribute(foamJumpStop, 1),
    );
    geometry.setAttribute(
      'jumpAngleMod',
      new THREE.Float32BufferAttribute(foamJumpAngleMod, 1),
    );
